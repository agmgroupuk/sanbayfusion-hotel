...
export function ApplicationForm() {
  ...
  const addOnTotal = useMemo(() => {
    ...
    return configuration.selectedAddOns.reduce((sum, selected) => {
      const price = beverageAddOns.find((addOn) => addOn.category === selected.category)?.price ?? 0;
  ...
  function submit() {
    ...
    startSubmitting(async () => {
      const payload: MembershipApplicationInput = { configuration, ...form };
  ...
  return <form onSubmit={(event) => { event.preventDefault(); submit(); }} className="space-y-14">
    ...
    <section><p className="text-eyebrow text-gold">How should we contact you?</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{contactOptions.map((option) => <label key={option} className="flex items-center gap-3 rounded-sm border border-border/60 px-4 py-3 text-sm"><input type="checkbox" checked={form.contactPreferences.includes(option)} onChange={() => toggleContact(option)} className="size-4 accent-[var(--gold)]" />{option}</label>)}</div></section>
